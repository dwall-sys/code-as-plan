...
grep -rn "require(" get-shit-done/bin/lib/cap-*.cjs | grep -v "node:" | grep -v "require('./cap-" | grep -v "require('../"
...
|-------|---------------|---------------|-----------|
| CJS (`get-shit-done/bin/lib/*.cjs`) | `node:test` + `node:assert` | `c8` | 70% lines |
