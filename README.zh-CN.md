...
```bash
# Claude Code
npx cap-cc --claude --global   # 安装到 ~/.claude/
npx cap-cc --claude --local    # 安装到 ./.claude/

# OpenCode（开源，可用免费模型）
npx cap-cc --opencode --global # 安装到 ~/.config/opencode/

# Gemini CLI
npx cap-cc --gemini --global   # 安装到 ~/.gemini/

# Codex（以 skills 为主）
npx cap-cc --codex --global    # 安装到 ~/.codex/
npx cap-cc --codex --local     # 安装到 ./.codex/

# Copilot（GitHub Copilot CLI）
npx cap-cc --copilot --global  # 安装到 ~/.github/
npx cap-cc --copilot --local   # 安装到 ./.github/

# Cursor CLI
npx cap-cc --cursor --global   # 安装到 ~/.cursor/
npx cap-cc --cursor --local    # 安装到 ./.cursor/

# Antigravity（Google，以 skills 为主，基于 Gemini）
npx cap-cc --antigravity --global # 安装到 ~/.gemini/antigravity/
npx cap-cc --antigravity --local  # 安装到 ./.agent/

# 所有运行时
npx cap-cc --all --global      # 安装到所有目录
```
...
OpenCode、Gemini CLI 和 Codex 现在都已经通过 `npx cap-cc` 获得原生支持。
