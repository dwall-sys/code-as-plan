...
```bash
INIT=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" phase insert "${after_phase}" "${description}")
```
