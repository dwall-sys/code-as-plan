...
```bash
INIT=$(node "$HOME/.claude/cap/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "$HOME/.claude/cap/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
