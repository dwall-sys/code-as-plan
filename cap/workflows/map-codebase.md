...
```bash
INIT=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" agent-skills gsd-codebase-mapper 2>/dev/null)
```
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
