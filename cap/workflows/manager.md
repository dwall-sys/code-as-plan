...
```bash
INIT=$(node "$HOME/.claude/cap/bin/gsd-tools.cjs" init manager)
...
2. Run: node \"$HOME/.claude/cap/bin/gsd-tools.cjs\" init plan-phase {N}
...
2. Run: node \"$HOME/.claude/cap/bin/gsd-tools.cjs\" init execute-phase {N}
