...
```bash
INIT=$(node "$HOME/.claude/cap/bin/gsd-tools.cjs" init execute-phase "${PHASE}")
...
```bash
node "$HOME/.claude/cap/bin/gsd-tools.cjs" phases list --type summaries --raw
...
```bash
node ~/.claude/cap/bin/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
```bash
NODE_REPAIR=$(node "./.claude/cap/bin/gsd-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/cap/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
node "$HOME/.claude/cap/bin/gsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/cap/bin/gsd-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/cap/bin/gsd-tools.cjs" state record-metric \
...
node "$HOME/.claude/cap/bin/gsd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/cap/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$HOME/.claude/cap/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/cap/bin/gsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/cap/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/cap/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
