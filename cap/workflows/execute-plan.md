...
@~/.claude/cap/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" init execute-phase "${PHASE}")
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" phases list --type summaries --raw
...
See `~/.claude/cap/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node ~/.claude/cap/bin/cap-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See ~/.claude/cap/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/cap/bin/cap-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/cap/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/cap/templates/user-preferences.md` (section: setup). Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/cap/templates/summary.md`.
...
node "$HOME/.claude/cap/bin/cap-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/cap/bin/cap-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/cap/bin/cap-tools.cjs" state record-metric \
...
node "$HOME/.claude/cap/bin/cap-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/cap/bin/cap-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
