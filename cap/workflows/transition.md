...
```bash
TRANSITION=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" config-set workflow._auto_chain_active false
```
