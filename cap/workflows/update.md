...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/cap/VERSION" ] || [ -f "./$dir/cap/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/cap/VERSION"
    LOCAL_MARKER_FILE="./$dir/cap/workflows/update.md"
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "$HOME/$dir/cap/VERSION" ] || [ -f "$HOME/$dir/cap/workflows/update.md" ]; then
    GLOBAL_RUNTIME="$runtime"
    GLOBAL_VERSION_FILE="$HOME/$dir/cap/VERSION"
    GLOBAL_MARKER_FILE="$HOME/$dir/cap/workflows/update.md"
