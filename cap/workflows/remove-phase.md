...
```bash
INIT=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" init phase-op "${target}")
...
```bash
RESULT=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" phase remove "${target}" --force)
```
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
