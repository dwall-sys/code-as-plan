...
<required_reading>
@~/.claude/cap/references/verification-patterns.md
@~/.claude/cap/templates/verification-report.md
</required_reading>
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" roadmap get-phase "${phase_number}"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" verify key-links "$plan")
...
See ~/.claude/cap/templates/verification-report.md for complete template.
</step>
