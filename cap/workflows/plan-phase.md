...
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/cap/bin/gsd-tools.cjs" agent-skills gsd-researcher 2>/dev/null)
...
5. Commit:
```bash
node "$HOME/.claude/cap/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node "$HOME/.claude/cap/bin/gsd-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
```bash
PHASE_DESC=$(node "$HOME/.claude/cap/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
```bash
UI_PHASE_CFG=$(node "$HOME/.claude/cap/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "$HOME/.claude/cap/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/cap/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
2. Disable Nyquist with the exact command:
   `node "$HOME/.claude/cap/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
