...
```bash
INIT=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" agent-skills gsd-executor 2>/dev/null)
```
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "$HOME/.claude/cap/bin/cap-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
POST_PLAN_INDEX=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
PARENT_INFO=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
VERIFIER_SKILLS=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" agent-skills gsd-verifier 2>/dev/null)
```
...
Commit the file:
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
```bash
COMPLETION=$(node "$HOME/.claude/cap/bin/cap-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "$HOME/.claude/cap/bin/cap-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
